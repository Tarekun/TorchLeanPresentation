...
private def buildX : Spec.Tensor Float (.dim nSamples (.dim inDim .scalar)) :=
...
private def buildY (X : Spec.Tensor Float (.dim nSamples (.dim inDim .scalar))) :
...
-- Returns nSamples (x, y) pairs.  Mirrors generate_batch() in pytorch/data.py.
def generateData : _root_.Runtime.Autograd.Train.Dataset (sample.Supervised α σ τ) :=
